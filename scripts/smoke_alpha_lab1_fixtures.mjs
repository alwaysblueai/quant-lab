...
const BASE_URL = process.env.ALPHA_LAB_SMOKE_BASE_URL || "http://127.0.0.1:8766";
const OUT_DIR = resolve(
  process.env.ALPHA_LAB_SMOKE_OUT_DIR
  || process.env.ALPHA_LAB_SMOKE_OUTPUT_DIR
  || "tmp/alpha_lab1_smoke",
);
const DEBUG_PORT = Number(process.env.ALPHA_LAB_SMOKE_DEBUG_PORT || 9227);
...
async function waitForPageReady(cdp) {
  ...
  while (Date.now() < deadline) {
    const result = await cdp.send("Runtime.evaluate", {
      expression: `(() => {
        ...
        return {
          ...
          hasOverview: Boolean(document.querySelector(".artifact-overview-shell")),
          hasError: /Boot failed|加载失败|fixture failed/i.test(text),
    ...
    if (value && value.readyState === "complete" && value.hasOverview && value.textLength > 500) return value;
    if (value && value.hasError) return value;
    await sleep(300);
  }
  throw new Error("Timed out waiting for fixture overview to render");
}
...
async function analyzeLayout(cdp) {
  const expression = `(() => {
    ...
    const ignoreOverflow = (el) => {
      ...
      if (el.closest(".artifact-viewer-raw, .md-box, .run-table-wrap, .artifact-table-wrap")) return true;
      return false;
    };
    ...
    for (const el of Array.from(document.querySelectorAll(".artifact-chart-card,.metrics-screening-card,.tearsheet-section,.artifact-overview-shell"))) {
      if (!isVisible(el)) continue;
      const text = (el.innerText || el.textContent || "").replace(/\\s+/g, " ").trim();
      const hasVisual = Boolean(el.querySelector("svg,canvas,img,table,.artifact-line-chart,.artifact-bar-list,.diag-heatmap,.coverage-event-strip,.metrics-chip,.tearsheet-table,.metrics-screening-subgrid"));
      if (!text && !hasVisual) blankModules.push(labelFor(el));
      if (/暂无可展示|No data|Loading\\.\\.\\./i.test(text) && !el.closest("details:not([open])")) {
        blankModules.push(labelFor(el));
      }
    }
    return {
      ...
      blankModules: blankModules.slice(0, 20),
      placeholders: placeholders.slice(0, 20),
      quickVerdictCount: document.querySelectorAll(".quick-verdict-banner").length,
      executiveVerdictCount: document.querySelectorAll(".tearsheet-exec-summary").length,
      chartCardCount: document.querySelectorAll(".artifact-chart-card").length,
      metricCardCount: document.querySelectorAll(".metrics-screening-card").length,
...
async function captureScreenshot(cdp, outputPath) {
  ...
  const width = Math.ceil(Math.min(Math.max(contentSize.width, 1), 2200));
  const height = Math.ceil(Math.min(Math.max(contentSize.height, 1), 18000));
...
async function main() {
  ...
  try {
    ...
    const listing = await fetchJson(`${BASE_URL}/api/dev/alpha-lab1/overview-fixtures`);
    const fixtures = Array.isArray(listing.fixtures) ? listing.fixtures : [];
    if (!fixtures.length) throw new Error("No alpha-lab1 dev fixtures were returned");
    const results = [];
    for (const fixture of fixtures) {
      for (const viewport of VIEWPORTS) {
        const url = `${BASE_URL}/dev/alpha-lab1/overview-fixture?case=${encodeURIComponent(fixture.id)}`;
    ...
    for (const result of results) {
      const { layout } = result;
      const isQuick = result.fixtureId.includes("quick");
      const isFull = result.fixtureId.includes("full");
      ...
      if (isQuick && layout.quickVerdictCount < 1) failures.push({ result, issue: "missing quick verdict banner", details: layout.quickVerdictCount });
      if (isFull && layout.executiveVerdictCount < 1) failures.push({ result, issue: "missing executive verdict", details: layout.executiveVerdictCount });
      if (layout.chartCardCount < 1 && layout.metricCardCount < 1) failures.push({ result, issue: "no chart or metric cards rendered", details: layout });
    }
    const report = {
