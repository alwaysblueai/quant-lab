# Claude Desktop Factor Writeback Merge Prompt

把下面 prompt 粘贴到 Claude Desktop。它是自包含的最终合并 prompt：Claude
Desktop 不需要再打开 `single_factor_iteration_writeback_template.md`，也不需要用户手工
套模板。你只要在 prompt 末尾粘贴三类输入材料，Claude 就应在对话中直接输出完整的
`single_factor_iteration_writeback_v1` Markdown 写回稿文本，不应自行创建或修改任何文件。

```text
你现在要担任 alpha-lab 单因子优化的最终写回合并编辑器。

你会收到三类材料：
1. Web GPT 机制迭代总结：负责初始假设、候选演化、设计取舍、可复用思路。
2. Claude Desktop Stage2.5 迭代内容：负责中间优化、推理补强、候选收敛。
3. Codex GUI 实验总结：负责本地 artifact、每轮实验表现、metrics、PIT、hash、audit。

你的目标：
- 按 `single_factor_iteration_writeback_v1` 写成最终知识库回写稿。
- 直接在对话中输出完整 Markdown 写回稿；不要只输出提纲、建议或待填模板。
- 不要要求用户再手工套用 `single_factor_iteration_writeback_template.md`。
- 不要自行访问、创建、移动、修改或删除 vault / repo 文件；尤其不要直接写入
  `50_experiments/`。
- 输出要适合未来大模型检索、理解、迁移和再创造。
- 不保存原始对话全文，只保留浓缩后的研究经验。
- 把 `emergent_moves` 写成最重要的主回写字段，必须具体、可迁移、可执行。
- 把 `operative_claims` 写成弱观察，不能写成硬 kill 规则。
- 把 `negative_constraints` 写清楚，避免未来重复踩坑。
...
- 不引入 Level 3 / execution / fill simulation / implementability replay 语义。
