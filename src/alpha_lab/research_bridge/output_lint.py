...
# Hedging / escape verdicts that validation_kill_tests strict mode
# explicitly rejects.
...
# to validation_kill_tests, not signal_mapping.
...
def _lint_mechanism_discovery(text: str, mode: str) -> LintReport:
    ...
    mechanism_count = len(_MECHANISM_HEADING_PATTERN.findall(text))
...
def _lint_signal_mapping(text: str, mode: str) -> LintReport:
    ...
    _check_phrase_ban(
        ...
        detail_template=(
            ...
            "select a winner — defer to validation_kill_tests"
        ),
        violations=violations,
    )
