...
This is a validation layer, not a data-ingestion framework.  It only raises
``ValueError`` with an informative message; it does not repair or impute data.
"""
...
def validate_price_panel(df: pd.DataFrame) -> None:
    ...
    # --- Not empty ----------------------------------------------------------
    if df.empty:
        raise ValueError("Price panel is empty (zero rows).")
